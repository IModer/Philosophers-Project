...
void FloatingWindow::Render(GameModel* model)
{
    ...
    if (_field->GetId() != ROADANDELECTRICPOLE) {
        ...
        if (gf->IsOnFire()) {
            ...
            if (fd == nullptr) {
                DrawText("Oh no! There are no fire departments nearby!", fireRect.x, fireRect.y, 20, RED);
            } else if (fd->getACount() > 0) {
                DrawText(("There are no available firefighters! Available in: " + STR(fd->getACount()) + "s.").c_str(), fireRect.x, fireRect.y, 20, RED);
            } else {
