...
{
    public:
        RoadsAndElectricPoles(FIELD_TYPES id, INT_TOUPLE location, INT_TOUPLE size, int direction): id(id), location(location), size(size), direction(direction), Field(id,location,size) {}
        FIELD_TYPES id;
        INT_TOUPLE location;
        INT_TOUPLE size;
        int direction;
